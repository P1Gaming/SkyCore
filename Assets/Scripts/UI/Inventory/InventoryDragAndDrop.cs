...
namespace UI.Inventory
{
    ...
    public class InventoryDragAndDrop
    {
        ...
        public void FeedJelly(float amountToIncrease, Jellies.Parameters parameters)
        {
               parameters.IncreaseFoodSaturation(amountToIncrease);
        }
            ...
            else
            {
                ...
                if (_beingDragged.ItemStack.itemInfo.Name == "Red Berry")
                {
                    ...
                    if (Physics.Raycast(ray, out RaycastHit hit, 100, layerMask))
                    {
                        ...
                        {
                            FeedJelly(_berryItem.SaturationValue, hit.collider.GetComponent<Jellies.Parameters>());
                            hit.collider.GetComponent<DewInstantiate>().DewSpawn();
                        }
                    }
                    else
                    {
